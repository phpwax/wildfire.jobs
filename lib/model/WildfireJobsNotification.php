...
class Wildfirejobsnotification extends WaxEmail{
  ...
  protected function parse_template($template, $data_item, $prefix=""){
    ...
    foreach($email_cols as $ecol) if($ecol != "id") foreach($data_cols as $dcol) if($dcol != "id" && $template->columns[$dcol]) $template->$ecol = str_replace("%".$prefix.$dcol."%", $data_item->$dcol, $template->$ecol);
